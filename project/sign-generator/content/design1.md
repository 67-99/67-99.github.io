...
|类型|参数|用途|
|:---:|:---:|:---:|
|A|宽高比：1<br>字体宽：1/10 h|一般情况|
|B|宽高比：0.65<br>字体宽：1/6 h|公路命名编号标志<br>公路编号标志<br>出口编号标志<br>里程牌和百米牌中的字母和阿拉伯数字|
|C|宽高比：0.5<br>字体宽：1/10 h|指路标志箭杆上的公路编号的字母和阿拉伯数字|

- 阿拉伯数字高度应为汉字高度的1/3~1/2
- 小数点后的阿拉伯数字高度宜为汉字高度的1/2~2/3
...
![国标一般道路路牌](content/resource/general-sign1.png)
其中，对于环岛式等平交路口预告路牌开发了带柄绘制模块（见下），它可以根据文字内容所占据的空间，自动生成一个指向文字内容的柄，以实现不同方向的预告指引自动化。![开放环岛方向示意](content/resource/kaifang-roundabout.png)